...
export default function Parameters() {
  ...
  const handleCheckboxChange = (fuel) => {
    setSelectedFuels((prev) => ({
      ...prev,
      [fuel]: { checked: !prev[fuel]?.checked, selectedValue: prev[fuel]?.selectedValue },
    }));
  };

  const handleRadioChange = (fuel, selectedValue) => {
    setSelectedFuels((prev) => ({
      ...prev,
      [fuel]: { ...prev[fuel], selectedValue },
    }));
  };
                                    ...
                                    {DummydataForParameters[key]?.map((fuelItem) => {
                                      ...
                                      return (
                                        <div key={fuelItem} className="flex flex-col p-2 border rounded-lg">
                                          <Checkbox
                                            checked={selectedFuels[fuelItem]?.checked || false}
                                          ...
                                          {fuelData && selectedFuels[fuelItem]?.checked && (
                                            <Radio.Group
                                              className="ml-10 mt-2"
                                              value={selectedFuels[fuelItem]?.selectedValue}
                                              onChange={(e) => handleRadioChange(fuelItem, e.target.value)}
                                            >
